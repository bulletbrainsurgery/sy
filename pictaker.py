...
"X-Requested-With": "XMLHttpRequest", "DNT": "1", "Referer": "http://www.syrnia.com/game.php", "Accept-Encoding": "gzip, deflate",
"Accept-Language": "en-US,en;q=0.9", "Cookie": "gamescreenwidth=1024; PHPSESSID=lq1k2oacc1ivmvd4kamc54lfe6; Syrnia=neggie333"}
...
def getimg(code):
            ...
            if not os.path.isfile(filename):
                ...
                if 12000 < len(pic) < 20000:
                    ...
                    # else:    # write contents into an error log: try bytes, then text. 
                        ...
                        # try: 
                            # with open()
                    win32api.MessageBox(0, 'didn\'t work', 'error :(', 0x00001000)
    ...
    print('Total: {:.5f}. Avg: {:.5f}\nMin: {:.5f} Max: {:.5f}'.format(endtime-starttime, sum(times)/len(times), min(times), max(times)))

    
    win32api.MessageBox(0, 'do the captcha you sausage', 'pics saved', 0x00001000) 
    import denoise
    for pic in savelist:
        filename = pic[0]
